...
export const appeal = async (timestampLastUpdate: number,chainid: ArrayElement<typeof supportedChainIds>) => {
  ...
  for(const appeal of AppealableDisputesData.disputes){
    ...
    if (!MetaevidenceData || !MetaevidenceData.dispute)
      ...
      const msg = await formatMessge(metaEvidenceUri, appeal, chainid == 1? "ethereum" : "gnosis")
...
const formatMessge = async (metaEvidenceUri: string | undefined, appeal: any, network: string) => {
  let res;
  if (metaEvidenceUri){
  ...
  return `[Dispute ${appeal.disputeID}](https://court.kleros.io/cases/${appeal.disputeID}) on ${network} concluded it's current round!
