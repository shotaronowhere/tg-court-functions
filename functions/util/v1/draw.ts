...
export const draw = async (timestampLastUpdate: number,chainid: ArrayElement<typeof supportedChainIds>) => {
    const JurorsDrawn = await getKBSubgraphData(
        chainid,
        "JurorsDrawn",
    ...
    for(const drawnJuror of uniqueJurors){
        ...
        for (const tg_user of tg_users?.data!) {
            const tg_users_id = tg_user.tg_user_id as string;

            await queue.add(async () => {
                await axios.post(`https://api.telegram.org/bot${process.env.BOT_TOKEN}/sendMessage`, {
                    chat_id: tg_users_id,
                    text: formatMessge(drawnJuror, chainid),
                    parse_mode: 'Markdown',
...
const formatMessge = (drawnJuror: ArrayElement<JurorsDrawnQuery['draws']>, chainid: number) => {
    const shortAddress = drawnJuror.address.slice(0, 6) + "..." + drawnJuror.address.slice(-4)
    return `Juror *${shortAddress}* has been drawn in [dispute ${drawnJuror.disputeId}](https://court.kleros.io/cases/${drawnJuror.disputeId}) (*${chainid == 1 ? 'mainnet' : 'gnosis'}*).`
}
